...
let render_action_comp = function(title: string, content: string, sprite_url: string) {
    return (    <div className='home_action_comp'>
        <img src={sprite_url}></img>
        <h3>{title}</h3>
        <p>{content}</p>
        <button className='button'>進入測驗 {'>>'}</button>
    </div>);
}
...
export default function Home_View() {
    return (
        <div className='home_actions container'>
            ...
            <section>
                {render_action_comp('客體關係理論', '客體關係理論是一種心理學理論，認為我們早年和父母或照顧者的關係會影響我們的心靈和未來的人際關係。這些早期經歷會在我們心裡留下「印記」，影響我們如何看待和處理各種情感和人際互動。', case_study_img)}
